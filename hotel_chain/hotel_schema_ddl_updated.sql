...
-- Snowflake DDL Scripts - Optimized for Snowflake Data Cloud
-- Multi-Dataset Structure: Database > Schema > Tables with Schema Prefix

-- Create database and schema
CREATE DATABASE IF NOT EXISTS SYNTHETIC_DATA;
USE DATABASE SYNTHETIC_DATA;

CREATE SCHEMA IF NOT EXISTS HOTEL_CHAIN;
USE SCHEMA HOTEL_CHAIN;

-- 1. HOTEL_CHAIN_HOTELS TABLE
-- Master table containing all hotel properties
CREATE OR REPLACE TABLE HOTEL_CHAIN_HOTELS (
...
-- 2. HOTEL_CHAIN_ROOM_TYPES TABLE
-- Different room categories available across hotels
CREATE OR REPLACE TABLE HOTEL_CHAIN_ROOM_TYPES (
...
-- 3. HOTEL_CHAIN_ROOM_INVENTORY TABLE
-- Room inventory by hotel and room type
CREATE OR REPLACE TABLE HOTEL_CHAIN_ROOM_INVENTORY (
    HOTEL_ID VARCHAR(20),
    ROOM_TYPE_ID VARCHAR(20),
    ROOM_COUNT INTEGER NOT NULL,
    CREATED_TIMESTAMP TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    PRIMARY KEY (HOTEL_ID, ROOM_TYPE_ID)
    -- Note: Foreign key constraints removed for Snowflake compatibility
);

-- 4. HOTEL_CHAIN_CUSTOMERS TABLE
-- Customer master data
CREATE OR REPLACE TABLE HOTEL_CHAIN_CUSTOMERS (
...
-- 5. HOTEL_CHAIN_CORPORATE_ACCOUNTS TABLE
-- Corporate and group accounts
CREATE OR REPLACE TABLE HOTEL_CHAIN_CORPORATE_ACCOUNTS (
...
-- 6. HOTEL_CHAIN_RESERVATIONS TABLE
-- Main reservations/bookings table
CREATE OR REPLACE TABLE HOTEL_CHAIN_RESERVATIONS (
    ...
    BOOKING_SOURCE VARCHAR(50), -- Direct.com, Expedia, Booking.com, etc.
...
-- 7. HOTEL_CHAIN_DAILY_ROOM_RATES TABLE
-- Daily rate management and pricing
CREATE OR REPLACE TABLE HOTEL_CHAIN_DAILY_ROOM_RATES (
    HOTEL_ID VARCHAR(20),
    ROOM_TYPE_ID VARCHAR(20),
    RATE_DATE DATE,
    RATE_CODE VARCHAR(20),
...
-- 8. HOTEL_CHAIN_ANCILLARY_SERVICES TABLE
-- Additional services and amenities
CREATE OR REPLACE TABLE HOTEL_CHAIN_ANCILLARY_SERVICES (
...
-- 9. HOTEL_CHAIN_ANCILLARY_SALES TABLE
-- Sales of additional services
CREATE OR REPLACE TABLE HOTEL_CHAIN_ANCILLARY_SALES (
...
-- 10. HOTEL_CHAIN_EVENTS_CALENDAR TABLE
-- Local events that impact demand
CREATE OR REPLACE TABLE HOTEL_CHAIN_EVENTS_CALENDAR (
...
-- 11. HOTEL_CHAIN_COMPETITIVE_SET TABLE
-- Competitor hotel information
CREATE OR REPLACE TABLE HOTEL_CHAIN_COMPETITIVE_SET (
...
-- 12. HOTEL_CHAIN_REVENUE_SUMMARY TABLE
-- Daily revenue rollup for performance tracking
CREATE OR REPLACE TABLE HOTEL_CHAIN_REVENUE_SUMMARY (
    HOTEL_ID VARCHAR(20),
    BUSINESS_DATE DATE,
...
CREATE OR REPLACE VIEW VW_HOTEL_CHAIN_PERFORMANCE AS
SELECT 
    h.HOTEL_ID,
    h.HOTEL_NAME,
    h.BRAND,
...
JOIN HOTEL_CHAIN_REVENUE_SUMMARY rs ON h.HOTEL_ID = rs.HOTEL_ID;

CREATE OR REPLACE VIEW VW_HOTEL_CHAIN_CUSTOMER_BOOKING_HISTORY AS
SELECT 
    c.CUSTOMER_ID,
    c.FIRST_NAME,
    c.LAST_NAME,
    ...
    r.BOOKING_CHANNEL,
    r.GUEST_TYPE
FROM HOTEL_CHAIN_CUSTOMERS c
JOIN HOTEL_CHAIN_RESERVATIONS r ON c.CUSTOMER_ID = r.CUSTOMER_ID
JOIN HOTEL_CHAIN_HOTELS h ON r.HOTEL_ID = h.HOTEL_ID;

COMMENT ON DATABASE SYNTHETIC_DATA IS 'Multi-dataset synthetic data warehouse for analytics and testing';
COMMENT ON SCHEMA HOTEL_CHAIN IS 'Hotel chain sales and revenue management synthetic dataset';
