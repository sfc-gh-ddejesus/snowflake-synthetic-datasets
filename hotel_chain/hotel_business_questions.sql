...
FROM HOTEL_CHAIN_REVENUE_SUMMARY rs
JOIN HOTEL_CHAIN_HOTELS h ON rs.HOTEL_ID = h.HOTEL_ID
...
JOIN HOTEL_CHAIN_HOTELS h ON a.HOTEL_ID = h.HOTEL_ID
JOIN HOTEL_CHAIN_ANCILLARY_SERVICES s ON a.SERVICE_ID = s.SERVICE_ID
JOIN HOTEL_CHAIN_REVENUE_SUMMARY rs ON a.HOTEL_ID = rs.HOTEL_ID AND a.SALE_DATE = rs.BUSINESS_DATE
...
JOIN HOTEL_CHAIN_DAILY_ROOM_RATES drr ON h.HOTEL_ID = drr.HOTEL_ID AND hri.ROOM_TYPE_ID = drr.ROOM_TYPE_ID
...
WITH demand_analysis AS (
    ...
    FROM HOTEL_CHAIN_DAILY_ROOM_RATES drr
    JOIN HOTEL_CHAIN_HOTELS h ON drr.HOTEL_ID = h.HOTEL_ID
...
WITH corporate_performance AS (
    ...
    LEFT JOIN HOTEL_CHAIN_RESERVATIONS r ON ca.ACCOUNT_ID = r.CORPORATE_ACCOUNT_ID
    WHERE r.RESERVATION_STATUS = 'Completed'
    AND r.CHECK_IN_DATE >= DATEADD('year', -1, CURRENT_DATE())
...
JOIN HOTEL_CHAIN_HOTELS h ON e.CITY = h.CITY
JOIN HOTEL_CHAIN_REVENUE_SUMMARY rs ON h.HOTEL_ID = rs.HOTEL_ID
...
WITH efficiency_metrics AS (
    ...
    FROM HOTEL_CHAIN_HOTELS h
    JOIN HOTEL_CHAIN_REVENUE_SUMMARY rs ON h.HOTEL_ID = rs.HOTEL_ID
    JOIN HOTEL_CHAIN_RESERVATIONS r ON h.HOTEL_ID = r.HOTEL_ID AND rs.BUSINESS_DATE = r.CHECK_IN_DATE
...
future_months AS (
    ...
    FROM (SELECT HOTEL_ID, HOTEL_NAME, REGION FROM HOTEL_CHAIN_HOTELS) h
